...
const SignIn = () => {
    ...
    const handleSubmit = (e) => {
        ...
        signInWithEmailAndPassword(auth, formData.email, formData.password)
            .then((res) => {
                console.log(res)
                localStorage.setItem("signin_Data",res);
        ...
        <div className="bg-cover min-h-screen flex items-center justify-center" style={{ backgroundImage: `url('https://source.unsplash.com/1600x900/?nature,water')` }}>
            <div className="max-w-md w-full p-8 bg-white rounded-lg shadow-lg">
                ...
                <p className="mt-4 text-center text-sm text-gray-600">
                    Don't Have an Account? <span onClick={onSignup} className="cursor-pointer font-medium text-indigo-600 hover:text-indigo-500">Sign up</span>
                </p>
            </div>
        </div>
    );
